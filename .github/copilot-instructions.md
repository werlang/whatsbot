...
- Background delivery behavior belongs in `api/background/` and WhatsApp client lifecycle behavior belongs in `api/services/whatsapp-client-manager.js`.
...
## Web rules
- The web service is an SSR Mustache app with one scheduler page at `/` and readiness JSON at `/ready`.
...
- The scheduler page posts directly to the API and polls live session state for QR/readiness updates.
