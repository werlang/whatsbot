...
- **MySQL** stores scheduled messages, delivery lifecycle data, and the session id to access password registry used by the login and restore flow.
...
- Existing environments that stored session access data in JSON must also apply [api/data/mysql/migrations/20260415_create_whatsapp_session_access.sql](/Users/pablowerlang/Documents/Workspaces/whatsbot/api/data/mysql/migrations/20260415_create_whatsapp_session_access.sql) before restarting the API.
...
```bash
docker compose exec -T mysql sh -lc 'mysql -uroot -p"$MYSQL_ROOT_PASSWORD" "$MYSQL_DATABASE" < /tmp/20260415_create_whatsapp_session_access.sql'
```
